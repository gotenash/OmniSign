...
async function syncPlaylist(playlistData) {
    ...
    try {
        const meetingsRes = await axios.get(resolveMediaUrl(`/api/player/meetings/today?deviceId=${DEVICE_ID}&siteId=${playlistData.siteId || ''}`), { timeout: 5000 });
        if (meetingsRes.data && meetingsRes.data.meetings) {
            ...
            if (meetingsRes.data.rooms) {
                ...
                console.log(`[Sync Meetings] Pas de clé "rooms" dans les données de réunion.`);
            }
        }
    } catch (e) {}
...
socket.on('screen-command', (data) => {
    ...
    if (action === 'off') {
        ...
        wakeUpScreen(skipPlayerRelaunch);
    }
});

// Watchdog pour renvoyer périodiquement les signaux d'allumage si l'écran doit être actif
// Cela permet de rallumer la TV si un utilisateur l'a mise en veille avec la télécommande.
setInterval(() => {
    if (lastScreenState === 'on') {
        console.log("⏰ Watchdog : Maintien de l'écran allumé (CEC/DPMS)...");
        wakeUpScreen(true);
    }
